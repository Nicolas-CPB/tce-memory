...
CREATE TABLE IF NOT EXISTS api_keys (
  ...
  FOREIGN KEY (project_id, team_id) REFERENCES projects(id, team_id) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS audit_log (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CHECK (project_id IS NULL OR team_id IS NOT NULL),
  FOREIGN KEY (project_id, team_id) REFERENCES projects(id, team_id) ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE IF NOT EXISTS observation_generation_jobs (
  ...
  FOREIGN KEY (agent_event_id, project_id, team_id) REFERENCES agent_events(id, project_id, team_id) ON DELETE CASCADE ON UPDATE CASCADE,
  FOREIGN KEY (project_id, team_id) REFERENCES projects(id, team_id) ON DELETE CASCADE ON UPDATE CASCADE
);
