...
class ServerBetaRuntimeInfoRoutes implements RouteHandler {
  ...
  setupRoutes(app: Application): void {
    ...
    app.get('/memory-ui/api/teams', async (req, res) => {
      try {
        const result = await pool.query('SELECT id,name,metadata,created_at,updated_at FROM teams ORDER BY name ASC');
    ...
    app.post('/memory-ui/api/teams', async (req, res) => {
      try {
        const name = String(req.body?.name ?? '').trim();
        if (!name) return void res.status(400).json({ error: 'BadRequest', message: 'name is required' });
        const existing = await pool.query('SELECT id,name,metadata,created_at,updated_at FROM teams WHERE name = $1 LIMIT 1', [name]);
        if (existing.rows[0]) return void res.status(200).json({ team: existing.rows[0], created: false });
